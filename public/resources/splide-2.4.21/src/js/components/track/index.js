...
export default (taco, Components) => {
	 ...
	 * @type {boolean}
	 */
	const isRTL = taco.options.direction === RTL;
	...
	const Track = {
		...
		shift() {
			...
			const right = abs(this.toPosition(taco.length));
		 ...
		 */
		trim(position) {
			if (!taco.options.trimSpace || taco.is(LOOP)) {
				return position;
			}

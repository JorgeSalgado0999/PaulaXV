 ...
 */
export default (taco, index, realIndex, slide) => {
	/**
	 * Whether to update "is-active" class before or after transition.
	 *
	 * @type {boolean}
	 */
	const updateOnMove = taco.options.updateOnMove;
	...
	const Slide = {
		...
		mount() {
			...
			Splide.on(STATUS_UPDATE_EVENTS, () => this.update())
				.on(STYLE_RESTORE_EVENTS, restoreStyles)
				.on("click", () => taco.emit("click", this), slide);
		...
		isVisible() {
			...
			if (taco.is(FADE) || active) {
				return active;
			}

			const { ceil } = Math;
			const trackRect = getRect(taco.Components.Elements.track);
			const slideRect = getRect(slide);

			if (taco.options.direction === TTB) {
				return (

...
export default (taco, Components) => {
	...
	const Controller = {
		...
		toIndex(page) {
			...
			index =
				index - (this.pageLength * perPage - length) * floor(index / length);
		 ...
		 */
		get nextIndex() {
			let next = taco.index + 1;

			if (isLoop || options.rewind) {
				next = this.rewind(next);
			}

			return (taco.index < next && next <= this.edgeIndex) || next === 0
				? next
				: -1;
		},
	};
	...
	function bind() {
		Splide.on("move", (newIndex) => {
			...
			Splide.index = between(taco.index, 0, Controller.edgeIndex);
		});
	}
